...
template <std::size_t N> class ArrayDifferentiator {
  public:
    ...
    explicit ArrayDifferentiator(const DifferentiatorOptions& options) { setOptions(options); }
    ...
    const Differentiator& axis(std::size_t index) const { return differentiators_[index]; }

    Differentiator& axis(std::size_t index) { return differentiators_[index]; }

  private:
    std::array<Differentiator, N> differentiators_{};
};

template <std::size_t N> class ArrayPositionVelocityLuenbergerObserver {
  public:
    ArrayPositionVelocityLuenbergerObserver() = default;

    explicit ArrayPositionVelocityLuenbergerObserver(const PositionVelocityObserverOptions& options) {
        setOptions(options);
    }

    void setOptions(const PositionVelocityObserverOptions& options) {
    ...
    const PositionVelocityLuenbergerObserver& axis(std::size_t index) const { return observers_[index]; }

    PositionVelocityLuenbergerObserver& axis(std::size_t index) { return observers_[index]; }
